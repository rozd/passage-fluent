...
struct CreateRefreshTokenModel<UserModel: PassageUserModel>: AsyncMigration {
    ...
    func prepare(on database: any Database) async throws {
        try await Self.createSchema(on: database, withSessionId: false)
    }

    static func createSchema(on database: any Database, withSessionId: Bool) async throws {
        let builder = database.schema(RefreshTokenModel<UserModel>.schema)
            .id()
        ...
        if withSessionId {
            builder.field("session_id", .uuid, .required)
        }
